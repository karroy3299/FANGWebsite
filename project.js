...
function renderDetail(project) {
  // 브라우저 탭 제목
  document.title = project.title + ' — FANGYUHAN';
  ...
  // 영상 처리
      if (project.video) {
        ...
        if (project.video.includes("youtube.com")) {
          // 유튜브 영상일 경우 iframe
          videoDiv.innerHTML = `<iframe width="560" height="315" src="${project.video}" frameborder="0" allowfullscreen></iframe>`;
        } else {
          // mp4 파일일 경우 video 태그
          videoDiv.innerHTML = `<video controls width="560"><source src="${project.video}" type="video/mp4"></video>`;
        }
      }
    };
